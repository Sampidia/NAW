...
/**
 * Game engine for MEDIUM level - captures pits with exactly 3 seeds
 */
class MediumGameEngine {
    ...
    /**
     * MEDIUM LEVEL: Captures pits with exactly 3 seeds if sowing created 2→3 transition
     */
    private fun handleCaptures(originalPits: IntArray, currentPits: IntArray, lastPitIndex: Int, player: Int): CaptureResult {
        ...
        // Check if sowing created any 2→3 transition
        var captureEnabled = false
        for (i in 0 until 12) {
            if (isOpponentPit(i, player) &&
                originalPits[i] + 1 == 3 && currentPits[i] == 3) {
                captureEnabled = true
                break
            }
        }
        ...
        if (captureEnabled) {
            // Capture opponent pits that are now exactly 3 seeds and were increased by sowing
            for (i in 0 until 12) {
                if (isOpponentPit(i, player) &&
                    currentPits[i] == 3 &&
