...
/**
 * Game engine for EASY level - captures pits with 2 or 3 seeds
 */
class EasyGameEngine {
    ...
    /**
     * EASY LEVEL: Captures pits with 2 or 3 seeds if sowing created a transition
     */
    private fun handleCaptures(originalPits: IntArray, currentPits: IntArray, lastPitIndex: Int, player: Int): CaptureResult {
        ...
        // Check if sowing created any transition that enables capture (1→2 or 2→3)
        var captureEnabled = false
        for (i in 0 until 12) {
            if (isOpponentPit(i, player) && originalPits[i] + 1 in 2..3 && currentPits[i] in 2..3) {
                captureEnabled = true
                break
            }
        }
        ...
        if (captureEnabled) {
            // Capture opponent pits that are now 2 or 3 seeds and were increased by sowing
            for (i in 0 until 12) {
                if (isOpponentPit(i, player) && currentPits[i] in 2..3 && originalPits[i] < currentPits[i]) {
